"""!
@brief internal implementation used for scheduling a task DAG with any of the HEFT family of algorithms
"""
...
class RankMetric(Enum):
    """!
    @brief The RankMetric class is an Enum that can be used to select which rank metric we should use to prioritize the nodes in the incoming DAG prior to processor allocation

    There are four different rank metrics available.\n
    - MEAN: Prioritize tasks based on average estimates for computation time\n
    - WORST: Prioritize tasks based on worst case estimates for computation time\n
    - BEST: Prioritize tasks based on best case estimates for computation time\n
    - EDP: Prioritize tasks based on average estimates for EDP impact
    """
    MEAN = "MEAN"  
    WORST = "WORST" 
    BEST = "BEST" 
...
class OpMode(Enum):
    """!
    @brief The OpMode class is an Enum that can be used to select which method should be used to perform processor allocation decisions based on the sorted list of tasks
    
    There are four different operational modes available.\n
    - EFT: Schedule tasks to the processor that gives them their earliest finish time.\n
    - EDP_REL: Schedule tasks to the processor that minimizes their estimated EDP with some tweaks in place to encourage load balancing.\n
    - EDP_ABS: Schedule tasks to the processor that minimizes their estimated EDP with no regard for load balancing.\n
    - ENERGY: Schedule tasks to the processor that minimizes their estimated total energy consumption.
    """
    EFT = "EFT"
...
def schedule_dag(dag, computation_matrix=W0, communication_matrix=C0, proc_schedules=None, time_offset=0, relabel_nodes=True, rank_metric=RankMetric.MEAN, **kwargs):
    ...
    @param proc_schedules: if existing schedules need to be passed in to HEFT (i.e. we are not scheduling onto an idle system), then they can be passed in as fixed constraints here
    ...
    @param rank_metric: specify the RankMetric enum to be used in scheduling this DAG
    @param kwargs: optional kwargs (op_mode - can be used to specify the operating mode, power_dict - contains a matrix of PE to power cost estimates used to inform power-aware scheduling decisions)
    """
...
def generate_argparser():
    """!
    For when this heft library is being invoked as a standalone commandline application, generate an argparser
    """
