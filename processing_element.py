    ...
    def run(self, sim_manager, task, resource, DVFS_module):
        ...
        try:
            with self.process.request() as req:                                 # Requesting the resource for the task
                ...
                if 'DAP' in self.name:
                    # Add the DAP reconfiguration overhead
                    task_runtime_max_freq += common.ClusterManager.cluster_list[self.cluster_ID].reconfiguration_overhead
                ...
                while task_complete is False:
                    ...
                    if self.type == 'ACC':
                        ...
                        dynamic_power = DTPM_power_models.compute_dynamic_power_dissipation(common.ClusterManager.cluster_list[self.cluster_ID].current_frequency,
                                                                                            ...
                                                                                            self.Cdyn_alpha)
                        dynamic_energy +=  dynamic_power * simulation_step * 1e-9
                    ...
                    static_energy += current_leakage * simulation_step * 1e-9

                    if (common.simulation_mode == "performance" and self.env.now >= common.warmup_period) or common.simulation_mode == "validation":
                        energy_sample = (dynamic_power + current_leakage) * simulation_step * 1e-9
                ...
                if len(common.ClusterManager.cluster_list[self.cluster_ID].OPP) == 0:
                    total_energy_task = dynamic_power_max_freq_core * task_time * 1e-9
                else:
                ...
                if (task.tail):
                    ...
                    ## Add to tracker the total number of bits across the simulation
                    if (self.env.now >= common.warmup_period):
                        jobname = task.jobname
                        for index, job in enumerate(sim_manager.jobs.list) :
                            if job.name == jobname :
                                common.results.bits_processed += job.inp_data_in_bits
                            ## if job.name == jobname :
                        ## for index, job in enumerate(sim_manager.jobs.list) :
                    ## if (self.env.now >= common.warmup_period):         
                    ...
                    common.results.total_completed_jobs += 1
                    if (self.env.now >= common.warmup_period):
                    ...
                    if common.warmup_based_on_jobs and common.results.total_completed_jobs == common.warmup_number_of_jobs:
                        common.warmup_period = self.env.now
                ...
                # If executing on DAP, update the configuration (remove)
                DASH_acc_utils.remove_config_current_exec_acc(self, resource, task)
                ...
                if (task.tail) and (self.env.now >= common.warmup_period) and (common.results.completed_jobs % common.snippet_size == 0):
                    ...
                    DTPM_utils.create_dataset_IL_DTPM(self.env.now, sim_manager.PEs)

                    if common.enable_real_time_constraints:
                        snippet_exec_time = (self.env.now - common.snippet_start_time) * 1e-9
