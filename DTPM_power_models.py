...
def compute_static_power_dissipation(cluster_ID, input_temperature=None, input_voltage=None):
    ...
    if common.ClusterManager.cluster_list[cluster_ID].type == "ACC":
        static_power_core = 0
        for config in common.ClusterManager.cluster_list[cluster_ID].current_acc_kernels_exec:
            acc_info = get_acc_config_info(common.ClusterManager.cluster_list[cluster_ID].name, config)
            static_power_core += acc_info.leakage_power
        static_power_core = static_power_core / common.ClusterManager.cluster_list[cluster_ID].num_total_cores
    else: # BIG and LTL
        ...
        static_power_core = static_power_cluster / common.ClusterManager.cluster_list[cluster_ID].num_total_cores
...
def get_max_power_consumption(cluster, PEs, N_tasks=None, N_cores=None):
    ...
    if num_tasks > 0:
        if (cluster.DVFS == 'imitation-learning' and common.enable_num_cores_prediction) or common.generate_complete_trace:
...
def get_acc_config_info(cluster_name, acc_config):
    '''!
    Get the accelerator configuration information.
    @param cluster_name: Name of the current cluster
    @param acc_config: Name of the current ACC configuration
    @return Configuration info for the current accelerator
    '''
    config = common.resource_matrix_Acc.dict.get(cluster_name + ',' + acc_config)
    if config == None:
        print("[E] Accelerator configuration was not found in the ACC_model.csv. Please check the SoC config file and the CSV file for inconsistencies.")
        print("[E] Config:", cluster_name, '-', acc_config)
        sys.exit()
    return config
#end get_acc_config_info(cluster_name, acc_config):
...
def predict_temperature_N_steps(power_list_clusters):
    '''!
    Predict the temperature N_steps ahead.
    N_steps is defined in config_file.ini
    @param power_list_clusters: Input power list for all clusters
    @return Predicted temperature N steps ahead
    '''
    power_list = []
    power_list.append(P_mem) # Memory model
    power_list.append(P_GPU) # GPU model
