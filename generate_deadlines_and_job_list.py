'''!
@brief This file generates a csv file with the deadlines for each snippet, following the specified thresholds, and it generates a job_list with a list of snippets (defined by num_snippets).

More information on the deadline generation can be found in HiLITE (https://ieeexplore.ieee.org/document/9085952).
'''
...
def generate_bursts(burst_size, gap_size):
    '''!
    Create high-temperature bursts based on the given parameters, i.e., creates sequences of snippets that have a high temperature profile.
    @param burst_size: Size of the burst
    @param gap_size: Number of snippets with low temperature profile
    '''
    job_list_sample = []
    burst_index = 0
