"""!
This file contains a list of methods that can be used for merging lists of DAGs into a single DAG for scheduling by HEFT
"""
...
class MergeMethod(Enum):
    """!
    @brief The MergeMethod class is an Enum that can be used to select which merge method to use to merge DAGs together when passed in to merge_dags

    There are two different merge methods available.\n
    - COMMON_ENTRY_EXIT: merge graphs by directly connecting a dummy head and sink node between all heads and sinks of the incoming DAGs\n
    - RANKING_BASED: utilize ranking-based DAG merging where DAGs are merged by linking their head nodes with nodes in the other graph that have comparable rank values\n
    See also: heft.RankMetric
    """
