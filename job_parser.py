        ...
        if not(found_new_task):                                                 # new_task = common.Tasks()
            if current_line[0] == 'job_name':                               
                ...
                if len(current_line) == 2 :
                    print('[E] Application ' + new_job.name + ' does not include data size in the job file. The calculated throughput might be inaccurate. Exiting...\n')
                    exit()
                else :
                    new_job.inp_data_in_bits = int(current_line[2])
