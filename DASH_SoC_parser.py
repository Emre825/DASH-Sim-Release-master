...
def resource_parse(resource_matrix, file_name):
    ...
    for line in input_file:
        ...
        if not(found_new_resource):
            if (current_line[0] == 'SoC_mesh_information'):                     # The key word "SoC_mesh_information" is used to specify x and y dimensions of mesh
                common.NoC_x_dim = int(current_line[1])
                common.NoC_y_dim = int(current_line[2])

            elif (current_line[0] == 'add_new_resource'):                         # The key word "add_new_resource" implies that the config file defines a new resource
                ...
                common.ResourceManager.latency_matrix = 5*np.ones((16,16),dtype=np.int16)  # Initialize the latency matrix
                ...
                if common.generate_complete_trace is False:
                    ...
                    if len(common.ClusterManager.cluster_list) < common.num_PEs_TRACE:
                        ...
                        new_cluster.DVFS = current_line[12]
        ...
        # end of: if not(found_new_resource)
        ...
        else: # if found_new_resource (i.e., found a new resource)
            ...
            if current_line[0] == 'opp':
            ...
            elif current_line[0] == 'acc_supported_configs':
                for i, config in enumerate(current_line):
                    if i != 0:
                        common.ClusterManager.cluster_list[cluster_ID].acc_config_list.append(config)
                ...
                for ii in range(capacity):
                    ...
                    resource_matrix.list[ind_PE].position = int(current_line[2])
                    ...
                    if 'LLC' in current_line[1] :
                        common.cache_positions.append((new_resource.ID, int(current_line[2])))
            else:
                for ii in range(capacity):
                    ...
                    if (each_PE_functionality > len(resource_matrix.list[ind_PE].supported_functionalities)):
                        if resource_matrix.list[ind_PE].type == 'ACC':
                            if len(current_line) == 2:
                                print("[E] The ACC configuration is not defined for the given task. Please check the SoC config file.")
                                print("[E] Task: " + line)
                                sys.exit()

                        if resource_matrix.list[ind_PE].type == 'ACC' and (current_line[2] in common.ClusterManager.cluster_list[cluster_ID].acc_config_list)\
                                or resource_matrix.list[ind_PE].type != 'ACC':
                            ...
                            resource_matrix.list[ind_PE].supported_functionalities.append("Task-not-supported")
                            print('[W] Configuration {} of task {} is not supported by accelerator {} (ID {})'.format(current_line[2], current_line[0], common.ClusterManager.cluster_list[cluster_ID].name, cluster_ID))
                        ...
                        if resource_matrix.list[ind_PE].type == 'ACC':
                            resource_matrix.list[ind_PE].DAP_config_list.append(current_line[2])
