...
def DyPO_policy(resource_matrix, PEs, timestamp) :
    '''!
    DyPO policy (https://dl.acm.org/doi/10.1145/3126530)
...
def imitation_learning_policy(resource_matrix, PEs, timestamp):
    '''!
    HiLITE IL policy (https://ieeexplore.ieee.org/document/9085952)
    @param resource_matrix: Resource matrix comprising all PEs
    @param PEs: The PEs available in the current SoC
    @param timestamp: Current timestamp
    '''
...
def aggregate_data(dataset_name, system_state, oracle):
    '''!
    DAgger algorithm - used by the IL-based policies.
    Aggregate sample to the dataset.
    @param dataset_name: Name of the dataset
    @param system_state: Sample to be saved (current system state)
    @param oracle: Oracle decision for the given sample
    '''
    if common.leave_one_out_experiments:
