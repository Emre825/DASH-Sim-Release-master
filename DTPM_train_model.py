'''!
@brief This file contains the training methods for the ML-based DTPM policies.
'''
import sys
import pandas as pd
import numpy as np
...
# Dataset selection
leave_one_out_experiments = common.leave_one_out_experiments
...
def train_model(X, y, pickle_file_name):
    '''!
    Based on the chosen ML algorithm configured in config_file.ini, train the DTPM policy.
    Different ML algorithms are supported, such as logistic regression, decision tree, and multi-layer perceptron.
    @param X: Input features
    @param y: Oracle
    @param pickle_file_name: Name of the DTPM policy to be saved
    '''
    if pickle_file_name == common.DTPM_regression_policy_file or not common.leave_one_out_experiments:
